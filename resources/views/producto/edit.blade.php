                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            @if($producto->codigo)
                                <span class="input-group-text codigo-first-letter" id="first-letter">
                                    {{ substr($producto->codigo, 0, 1) }}
                                </span>
                                <input type="text" 
                                    name="codigo" 
                                    id="codigo"
                                    class="form-control"
                                    value="{{ old('codigo', substr($producto->codigo, 1)) }}"
                                    placeholder="Resto del código">
                            @else
                                <span class="input-group-text codigo-first-letter" id="first-letter">S</span>
                                <input type="text" 
                                    name="codigo" 
                                    id="codigo"
                                    class="form-control"
                                    value="{{ old('codigo') }}"
                                    placeholder="Ingrese el código (sin la S inicial)">
                            @endif
                        </div>
                        <small class="form-text text-muted">La primera letra "S" es automática y no se puede modificar</small>
                        @error('codigo')
...
<script>
    ...
    // Manejar el código para agregar automáticamente la "S" al enviar el formulario
    document.querySelector('form').addEventListener('submit', function(e) {
        const codigoInput = document.getElementById('codigo');
        const firstLetter = document.getElementById('first-letter').textContent;
        
        // Si hay valor en el input, agregar la primera letra automática
        if (codigoInput.value.trim() !== '') {
            codigoInput.value = firstLetter + codigoInput.value.trim();
        }
    });
</script>
@endpush